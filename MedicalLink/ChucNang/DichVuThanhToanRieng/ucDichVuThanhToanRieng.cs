            ...
            try
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    if (cboKieuCapNhat.Text == "Đi kèm sang hao phí PTTT")
                    {
                        ...
                        //Log
                        sqlinsert_log = "INSERT INTO tools_tbllog(loguser, logvalue, ipaddress, computername, softversion, logtime, logtype) VALUES ('" + SessionLogin.SessionUsercode + "', 'Cập nhật thành công đối tượng thanh toán Đi kèm sang Hao phí PTTT của dịch vụ/thuốc/vật tư SL=" + gridViewDSDichVu.RowCount + "', '" + SessionLogin.SessionMyIP + "', '" + SessionLogin.SessionMachineName + "', '" + SessionLogin.SessionVersion + "', '" + dateupdate + "', 'TOOL_19');";
                    }
                    else if (cboKieuCapNhat.Text == "VTTT riêng không có ID DV đi kèm")
                    {
