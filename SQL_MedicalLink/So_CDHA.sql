...
SELECT
		...
		nth.username AS nguoithuchien,
		nd.username as nguoiduyet
FROM
	(SELECT mbp.maubenhphamid,
			mbp.hosobenhanid,
			mbp.departmentgroupid,
			...
			se.servicevalue as ketqua,
			se.servicedoer,
			se.servicecomment || 'UP_OK:'
	FROM maubenhpham mbp
		LEFT JOIN service se ON se.maubenhphamid=mbp.maubenhphamid
	WHERE se.servicecode not in (select sef.servicegroupcode from service_ref sef group by sef.servicegroupcode) and mbp.maubenhphamgrouptype=0 and
	mbp.departmentid_des='" + cboPhongThucHien.EditValue.ToString() + "' and " + tieuchi + ">='" + datetungay + "' and " + tieuchi + "<='" + datedenngay + "' 
		) A	
	INNER JOIN hosobenhan hsba ON hsba.hosobenhanid=A.hosobenhanid
	LEFT JOIN departmentgroup kyc ON kyc.departmentgroupid=A.departmentgroupid
	LEFT JOIN department pyc ON pyc.departmentid=A.departmentid and pyc.departmenttype in (2,3,9)
	LEFT JOIN tools_tblnhanvien nth ON nth.usercode=A.servicedoer
	LEFT JOIN tools_tblnhanvien nd ON nd.usercode=A.servicecomment;
