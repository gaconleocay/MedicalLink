...
{
    public partial class DM_DonViTinh : UserControl
    {
        ...
        private void LoadDanhSachDVT()
        {
            try
            {
                string _sqlDSKho = $@"select row_number () over (order by donvitinhname) as stt,* from pm_donvitinh WHERE isremove=0;";
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    _sqlinsert = "UPDATE pm_donvitinh SET donvitinhname='" + txtdonvitinh.Text.Trim() + "', islock='" + (chkIslock.Checked == true ? 1 : 0) + "', lastuserupdated='" + Base.SessionLogin.SessionUsercode + "' WHERE donvitinhid=" + this.selectID + ";";
                }
                else
                {
                    _sqlinsert = "INSERT INTO pm_donvitinh(donvitinhname, lastuserupdated) VALUES ('" + txtdonvitinh.Text.Trim() + "', '" + Base.SessionLogin.SessionUsercode + "');";
                }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string _sqlXoa = "DELETE FROM pm_donvitinh WHERE donvitinhid=" + this.selectID + ";";
        ...
        private void gridViewData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtDVTID.Text = this.selectID.ToString();
                txtdonvitinh.Text = gridViewData.GetRowCellValue(rowHandle, "donvitinhname").ToString();
