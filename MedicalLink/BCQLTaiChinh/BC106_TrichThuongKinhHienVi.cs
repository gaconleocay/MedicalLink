...
{
    public partial class BC106_TrichThuongKinhHienVi : UserControl
    {
        #region Khai bao
        ...
        public BC106_TrichThuongKinhHienVi()
        {
            InitializeComponent();
        }

        #region Load
        private void ucBC106_TrichThuongKinhHienVi_Load(object sender, EventArgs e)
        {
            try
            {
                LoadDuLieuMacDinh();
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string sql_timkiem = @"SELECT row_number() over() as stt, 'Ngoại sọ não (F12)' as departmentgroupname, sum(ser.soluong) as soluong, sum(ser.soluong*400000) as sotien, 0 as truthue, sum(ser.soluong*400000) as tongtien, '' as ghichu, '' as kynhan FROM (select vienphiid,departmentid,soluong,servicepricecode,servicepricename,billid_thutien,billid_clbh_thutien, (case when doituongbenhnhanid=4 then servicepricemoney_nuocngoai else (case when loaidoituong=0 then servicepricemoney_bhyt when loaidoituong=1 then servicepricemoney_nhandan else servicepricemoney end) end) as dongia from serviceprice where bhyt_groupcode='06PTTT' and (case when loaidoituong>0 then billid_thutien>0 or billid_clbh_thutien>0 end) " + tieuchi_ser + lstdichvu_ser + ") ser inner join (select vienphiid,vienphistatus from vienphi where 1=1 " + tieuchi_vp + trangthai_vp + ") vp on vp.vienphiid=ser.vienphiid";
        ...
        private void tbnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (radioXemTongHop.Checked)
                {
                    ...
                    string fileTemplatePath = "BC_106_TrichThuongKinhHienVi.xlsx";
        ...
        private void gridViewDataBC_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                ...
                if (columeFieldName == "truthue")
                {
                    frmBC106_NhapTruThue frmCauHinh = new frmBC106_NhapTruThue();
                    frmCauHinh.MyGetData = new frmBC106_NhapTruThue.GetTongChi(GetTruThueFunction);
        ...
        public void GetTruThueFunction(decimal _truthue)
        {
            try
            {
                ...
                foreach (var item in this.lstBaoCao)
                {
                    if (item.stt == _stt)
                    {
                        item.truthue = _truthue;
                        item.tongtien = item.sotien - item.truthue;
                    }
                }
                gridControlDataBC.DataSource = null;
                gridControlDataBC.DataSource = this.lstBaoCao;
        ...
        //            if (item.stt == _stt)
        //            {
        //                item.truthue= Utilities.Util_TypeConvertParse.ToDecimal(gridViewDataBC.GetRowCellValue(rowHandle, "truthue").ToString());
        //                item.tongtien = item.sotien - item.truthue;
        //            }
        //        }
