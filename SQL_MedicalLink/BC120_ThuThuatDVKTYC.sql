...
SELECT (row_number() OVER (PARTITION BY degp.departmentgroupid order by degp.departmentgroupname,ser.servicepricename)) as stt,
	...
	sum(ser.soluong*ser.dongia*0.98) as thanhtiensauthue,
	(sum(ser.soluong*ser.dongia*0.98)*0.5) as trichlaibv,
	(sum(ser.soluong*ser.dongia*0.98)*0.45) as khoathuchien,
	(sum(ser.soluong*ser.dongia*0.98)*0.02) as banlanhdao,
	(sum(ser.soluong*ser.dongia*0.98)*0.01) as tckt,
	(sum(ser.soluong*ser.dongia*0.98)*0.01) as cntt,
	(sum(ser.soluong*ser.dongia*0.98)*0.01) as khth,
	0 as isgroup
...
GROUP BY degp.departmentgroupid,degp.departmentgroupname,ser.servicepricecode,ser.servicepricename,ser.dongia;
