...
{
    /// <summary>
    /// BC Doanh thu theo Phòng chỉ định dịch vụ
    /// </summary>
    public partial class ucBaoCaoBenhNhanNgoaiTru : UserControl
    {
        ...
        #region Load
        public ucBaoCaoBenhNhanNgoaiTru()
        {
            InitializeComponent();
        }

        private void ucBaoCaoBenhNhanNgoaiTru_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDataDSPhongKham()
        {
            try
            {
                ...
                if (dsPhongKham != null && dsPhongKham.Count > 0)
                {
                    ...
                    {
                        BCDashboardBenhNhanNgoaiTru phongkham = new BCDashboardBenhNhanNgoaiTru();
                        phongkham.departmentid = Utilities.Util_TypeConvertParse.ToInt64(dsPhongKham[i]["departmentid"].ToString());
                        phongkham.departmentcode = dsPhongKham[i]["departmentcode"].ToString();
                        phongkham.departmentname = dsPhongKham[i]["departmentname"].ToString();
                        dataDSPhongKham.Add(phongkham);
                    }
                }
            }
            catch (Exception ex)
