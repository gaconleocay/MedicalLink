            ...
            try
            {
                ...
                foreach (var item_dv in this.lstDVXetNghiemChiPhi)
                {
                    if (item_dv.STT != null)
                    {
                        ...
                        try
                        {
                            string _sqlkiemtratrung = "SELECT * FROM ml_mayxnchiphi WHERE mayxn_ma='" + item_dv.MAYXN_MA + "' and servicepricecode='" + item_dv.SERVICEPRICECODE + "'; ";
                            DataTable _dataKiemTra = condb.GetDataTable_MeL(_sqlkiemtratrung);
                            if (_dataKiemTra != null && _dataKiemTra.Rows.Count > 0)
                            {
                                //Xoa
                                string _sql_delete = "DELETE FROM ml_mayxnchiphi WHERE mayxn_ma='" + item_dv.MAYXN_MA + "' and servicepricecode='" + item_dv.SERVICEPRICECODE + "'; ";
                                if (condb.ExecuteNonQuery_MeL(_sql_delete))
                                {
                                    _update_count += _dataKiemTra.Rows.Count;
                                }
                            ...
                            sql_insert = "INSERT INTO ml_mayxnchiphi(mayxn_ma, mayxn_ten, servicepricecode, servicepricename, servicepriceunit, cp_hoachat, cp_haophixn, cp_luong, cp_diennuoc, cp_khmaymoc, cp_khxaydung, lastuserupdated, lasttimeupdated) VALUES ('" + item_dv.MAYXN_MA + "', '" + item_dv.MAYXN_TEN + "', '" + item_dv.SERVICEPRICECODE + "', '" + _SERVICEPRICENAME + "', '" + item_dv.SERVICEPRICEUNIT + "', '" + _cp_hoachat + "', '" + _cp_haophixn + "', '" + _cp_luong + "', '" + _cp_diennuoc + "', '" + _cp_khmaymoc + "', '" + _cp_khxaydung + "', '" + Base.SessionLogin.SessionUsercode + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'); ";
                            if (condb.ExecuteNonQuery_MeL(sql_insert))
                            {
                                insert_count += 1;
                            }
