...
SELECT 
	tmp.stt,
	--(case when tmp.stt=1 then tmp.stt::text else (tmp.stt::text || '.' || (row_number() OVER (PARTITION BY tmp.tools_othertypelistname))) end) as stt1,
	(case when tmp.stt=1 then tmp.tools_othertypelistname else tmp.tools_otherlistname end) as tools_othertypelistname
FROM
	(select (row_number() OVER (PARTITION BY ot.tools_othertypelistname ORDER BY o.tools_otherlistname)) as stt,
		ot.tools_othertypelistid,
		ot.tools_othertypelistname,
		o.tools_otherlistid,
		o.tools_otherlistname
	from tools_othertypelist ot
		inner join tools_otherlist o on o.tools_othertypelistid=ot.tools_othertypelistid) tmp
	
---- Lấy DS danh mục CĐHA có 2 kết quả trở lên
...
select
	...
	se.servicecode,
	se.servicename
from (select * from servicepriceref where ServiceGroupType=3 and servicelock=0 and coalesce(isremove,0)=0) serf 
		inner join serviceref4price map on map.servicepricecode=serf.servicepricecode
		inner join service_ref se on se.servicecode=map.servicecode
where map.servicepricecode in (select tmp.servicepricecode from (select (row_number() OVER (PARTITION BY servicepricecode)) as stt,servicepricecode from serviceref4price) tmp where tmp.stt>=2)	
