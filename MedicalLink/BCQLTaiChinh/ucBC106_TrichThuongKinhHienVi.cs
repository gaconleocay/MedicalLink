    ...
    public partial class ucBC106_TrichThuongKinhHienVi : UserControl
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                sql_timkiem = @"SELECT row_number() over() as stt, 'Ngoại sọ não (F12)' as departmentgroupname, sum(ser.soluong) as soluong, sum(ser.soluong*400000) as sotien, 0 as truthue, sum(ser.soluong*400000) as tongtien, '' as ghichu, '' as kynhan FROM (select vienphiid,departmentid,soluong,servicepricecode,servicepricename, (case when doituongbenhnhanid=4 then servicepricemoney_nuocngoai else servicepricemoney_nhandan end) as dongia from serviceprice where bhyt_groupcode='06PTTT' " + tieuchi_ser + lstdichvu_ser + ") ser inner join (select vienphiid from vienphi where 1=1 " + tieuchi_vp + trangthai_vp + ") vp on vp.vienphiid=ser.vienphiid;";
