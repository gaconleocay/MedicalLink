...
--------
---Theo Chi tiet benh nhan ngay 8/5
...
--==========================CHi tiet theo khoa -  Tong hop toan vien - theo khoa chi dinh
...
FROM departmentgroup depg
	...
	(SELECT V.departmentgroupid,
		...
		(SELECT sum(b.datra) as tam_ung,
				...
				GROUP BY vp.vienphiid, b.departmentgroupid) V		
		GROUP BY V.departmentgroupid) A ON A.departmentgroupid=depg.departmentgroupid
	LEFT JOIN
	(SELECT spt.departmentgroupid,
		...
		sum(spt.money_khac_bh + spt.money_khac_vp) as money_khac, 		
		sum(spt.money_pttt_bh + spt.money_dvktc_bh + spt.money_thuoc_bh + spt.money_vattu_bh + spt.money_vtthaythe_bh + spt.money_xetnghiem_bh + spt.money_cdha_bh + spt.money_tdcn_bh + spt.money_khambenh_bh + spt.money_mau_bh + spt.money_giuongthuong_bh + spt.money_giuongyeucau_bh + spt.money_phuthu_bh + spt.money_vanchuyen_bh + spt.money_khac_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh + spt.money_nuocsoi_bh + spt.money_xuatan_bh + spt.money_diennuoc_bh + spt.money_pttt_vp + spt.money_dvktc_vp + spt.money_thuoc_vp + spt.money_vattu_vp + spt.money_vtthaythe_vp + spt.money_xetnghiem_vp + spt.money_cdha_vp + spt.money_tdcn_vp + spt.money_khambenh_vp + spt.money_mau_vp + spt.money_giuongthuong_vp + spt.money_giuongyeucau_vp + spt.money_phuthu_vp + spt.money_vanchuyen_vp + spt.money_khac_vp + spt.money_dkpttt_thuoc_vp + spt.money_dkpttt_vattu_vp + spt.money_nuocsoi_vp + spt.money_xuatan_vp + spt.money_diennuoc_vp) as money_tong		
		FROM tools_serviceprice_pttt spt WHERE spt.vienphistatus_vp=1 and spt.duyet_ngayduyet_vp between '" + dateTu + "' and '" + dateDen + "' GROUP BY spt.departmentgroupid) B ON B.departmentgroupid=depg.departmentgroupid WHERE depg.departmentgrouptype in (1,4,11,10,100)) O;
		
		
		
---Theo Tong hop ngay 15/6 - lay theo khoa phong chi dinh
SELECT ROW_NUMBER () OVER (ORDER BY O.loaivienphi) as stt,
		O.*
FROM		
(SELECT 
		(case de.departmenttype when 2 then 'Ngoại trú'
								when 3 then 'Nội trú'
								when 9 then 'Điều trị ngoại trú'
								else '' end) as loaivienphi,
...
FROM department de
	...
	(SELECT V.departmentid,
		...
		FROM
		(SELECT sum(b.datra) as tam_ung,
					(case when b.departmentid=0 then 8
							else b.departmentid end) as departmentid,
					vp.vienphiid,
					vp.doituongbenhnhanid
				FROM vienphi vp LEFT JOIN bill b on vp.vienphiid=b.vienphiid and b.loaiphieuthuid=2 and b.dahuyphieu=0
				WHERE vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + dateTu + "' and '" + dateDen + "'
				GROUP BY vp.vienphiid, b.departmentid) V		
	...
	(SELECT spt.departmentid,
		...
		sum(spt.money_khac_bh + spt.money_khac_vp) as money_khac, 		
		sum(spt.money_pttt_bh + spt.money_dvktc_bh + spt.money_thuoc_bh + spt.money_vattu_bh + spt.money_vtthaythe_bh + spt.money_xetnghiem_bh + spt.money_cdha_bh + spt.money_tdcn_bh + spt.money_khambenh_bh + spt.money_mau_bh + spt.money_giuongthuong_bh + spt.money_giuongyeucau_bh + spt.money_phuthu_bh + spt.money_vanchuyen_bh + spt.money_khac_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh + spt.money_nuocsoi_bh + spt.money_xuatan_bh + spt.money_diennuoc_bh + spt.money_pttt_vp + spt.money_dvktc_vp + spt.money_thuoc_vp + spt.money_vattu_vp + spt.money_vtthaythe_vp + spt.money_xetnghiem_vp + spt.money_cdha_vp + spt.money_tdcn_vp + spt.money_khambenh_vp + spt.money_mau_vp + spt.money_giuongthuong_vp + spt.money_giuongyeucau_vp + spt.money_phuthu_vp + spt.money_vanchuyen_vp + spt.money_khac_vp + spt.money_dkpttt_thuoc_vp + spt.money_dkpttt_vattu_vp + spt.money_nuocsoi_vp + spt.money_xuatan_vp + spt.money_diennuoc_vp) as money_tong		
		FROM tools_serviceprice_pttt spt WHERE spt.vienphistatus_vp=1 and spt.duyet_ngayduyet_vp between '" + dateTu + "' and '" + dateDen + "' GROUP BY spt.departmentid) B ON B.departmentid=de.departmentid 
...
-----==========BÁO CÁO DOANH THU - TÍNH THEO KHOA CHỈ ĐỊNH

-- Chia theo khoa ngày 10/5
...
FROM departmentgroup depg
	...
	(SELECT spt.departmentgroupid,
		...
		WHERE spt.vienphistatus_vp=1 
			and spt.duyet_ngayduyet_vp between '" + dateTu + "' and '" + dateDen + "' 
		GROUP BY spt.departmentgroupid) B ON B.departmentgroupid=depg.departmentgroupid WHERE depg.departmentgrouptype in (1,4,11,10,100)) O;
