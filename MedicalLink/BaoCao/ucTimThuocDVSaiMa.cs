    ...
    public partial class ucTimThuocDVSaiMa : UserControl
    {
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string sql_timkiem = "SELECT row_number() over() as stt, serv.servicepriceid, serv.medicalrecordid, vp.patientid, serv.vienphiid, serv.hosobenhanid, serv.maubenhphamid, kcd.departmentgroupname, pcd.departmentname, serv.servicepricecode, serv.servicepricename, serv.servicepricename_bhyt, serv.servicepricemoney, serv.servicepricemoney_nhandan, serv.servicepricemoney_bhyt, serv.servicepricemoney_nuocngoai, serv.soluong,serv.servicepricedate FROM (SELECT ser.servicepriceid, ser.medicalrecordid, ser.vienphiid, ser.hosobenhanid, ser.maubenhphamid, ser.departmentid, ser.departmentgroupid, ser.servicepricecode, ser.servicepricename, ser.servicepricename_bhyt, ser.servicepricemoney, ser.servicepricemoney_nhandan, ser.servicepricemoney_bhyt, ser.servicepricemoney_nuocngoai, ser.soluong, ser.servicepricedate FROM serviceprice ser WHERE ser.servicepricedate between '" + datetungay + "' and '" + datedenngay + "' and ser.servicepricecode not in (select service_code from (select servicepricecode as service_code from servicepriceref where servicepricecode <>'' union all select medicinecode as service_code from medicine_ref where medicinecode <>'') A)) serv inner join (select vienphiid, patientid from vienphi) vp on vp.vienphiid=serv.vienphiid INNER JOIN (select departmentid,departmentname from department where departmenttype in (2,3,9)) pcd ON pcd.departmentid=serv.departmentid INNER JOIN (select departmentgroupid,departmentgroupname from departmentgroup) kcd ON kcd.departmentgroupid=serv.departmentgroupid;  ";
