...
---------Version 2.0
 --bao cao PTTT co tinh cho GMHT: ngay 2/4: chuyen sang dung bang luu trong trigger
 -- tach thuoc vat tu co tinh tien trong goi pttt
SELECT ROW_NUMBER () OVER (ORDER BY O.departmentgroupname) as stt, 
O.*, 
bill.tam_ung as tam_ung 
FROM 
	...
	LEFT JOIN 
		...
		sum(spt.money_pttt_bh + spt.money_dvktc_bh + spt.money_thuoc_bh + spt.money_vattu_bh + spt.money_vtthaythe_bh + spt.money_xetnghiem_bh + spt.money_cdha_bh + spt.money_tdcn_bh + spt.money_khambenh_bh + spt.money_mau_bh + spt.money_giuongthuong_bh + spt.money_giuongyeucau_bh + spt.money_phuthu_bh + spt.money_vanchuyen_bh + spt.money_khac_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh) as money_tong_bh,
		...
		WHERE spt.departmentid not in (34,335,269,285) and spt.vienphistatus_vp=1 " + doituongbenhnhanid_spt + "
		and spt.duyet_ngayduyet_vp >='" + thoiGianTu + "' and spt.duyet_ngayduyet_vp <='" + thoiGianDen + "' 
		GROUP BY spt.departmentgroupid) A ON dep.departmentgroupid=A.departmentgroupid 
	LEFT JOIN 
		(SELECT spt.departmentgroup_huong, 
