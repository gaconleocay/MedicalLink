	...
	LEFT JOIN	
			...
			FROM
				...
				from (select vp.vienphiid from vienphi vp where vp." + _loaitrangthai + " " + doituongbenhnhanid_vp + " and vp." + loaithoigian + " between '" + thoiGianTu + "' and '" + thoiGianDen + "' ) vp
					inner join (select servicepriceid,vienphiid,servicepricecode,soluong,departmentgroupid from serviceprice where bhyt_groupcode='03XN' and loaidoituong in (0,4,6)  and servicepricedate>'2017-05-01 00:00:00') ser on ser.vienphiid=vp.vienphiid
					inner join (select s.idmayxn,servicepriceid from service s where s.idmayxn>0 and s.servicedate>'2017-05-01 00:00:00') se on se.servicepriceid=ser.servicepriceid
				group by ser.servicepriceid,ser.servicepricecode,ser.soluong,se.idmayxn,ser.departmentgroupid) SERV
			INNER JOIN (select cp.mayxn_ma,cp.servicepricecode,(cp.cp_hoachat+cp.cp_haophixn+cp.cp_luong+cp.cp_diennuoc+cp.cp_khmaymoc+cp.cp_khxaydung) as chiphixn from ml_mayxnchiphi cp) chiphi on chiphi.servicepricecode=SERV.servicepricecode and chiphi.mayxn_ma=SERV.idmayxn
...
LEFT JOIN 
	(select sum(b.datra) as tam_ung, 
		b.departmentgroupid 
	from (select vienphiid from vienphi where vienphistatus_vp=1 " + doituongbenhnhanid_vp + " and duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "') vp 
	...
	LEFT JOIN	
		...
		(SELECT 
				...
				FROM
					(select  
						...
						ser.departmentgroupid
					from (select vp.vienphiid from vienphi vp where vp.vienphistatus_vp=1 " + doituongbenhnhanid_vp + " and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "') vp
						inner join (select servicepriceid,vienphiid,servicepricecode,soluong,departmentgroupid from serviceprice where bhyt_groupcode='03XN' and servicepricedate>'2017-05-01 00:00:00') ser on ser.vienphiid=vp.vienphiid
						inner join (select s.idmayxn,servicepriceid from service s where s.idmayxn>0 and s.servicedate>'2017-05-01 00:00:00') se on se.servicepriceid=ser.servicepriceid
					group by ser.servicepriceid,ser.servicepricecode,ser.soluong,se.idmayxn,ser.departmentgroupid) SERV
				INNER JOIN (select cp.mayxn_ma,cp.servicepricecode,(cp.cp_hoachat+cp.cp_haophixn+cp.cp_luong+cp.cp_diennuoc+cp.cp_khmaymoc+cp.cp_khxaydung) as chiphixn from ml_mayxnchiphi cp) chiphi on chiphi.servicepricecode=SERV.servicepricecode and chiphi.mayxn_ma=SERV.idmayxn
		...
		SELECT 
				...
				(select  
						...
						ser.departmentgroupid
					from (select vp.vienphiid from vienphi vp where vp.vienphistatus_vp=1 " + doituongbenhnhanid_vp + " and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "') vp
						inner join (select servicepriceid,vienphiid,servicepricecode,soluong,departmentgroupid,maubenhphamid from serviceprice where bhyt_groupcode='03XN' and servicepricedate>'2017-05-01 00:00:00') ser on ser.vienphiid=vp.vienphiid
				...
				INNER JOIN (select cp.mayxn_ma,cp.servicepricecode,(cp.cp_hoachat+cp.cp_haophixn+cp.cp_luong+cp.cp_diennuoc+cp.cp_khmaymoc+cp.cp_khxaydung) as chiphixn from ml_mayxnchiphi cp) chiphi on chiphi.servicepricecode=SERV.servicepricecode
