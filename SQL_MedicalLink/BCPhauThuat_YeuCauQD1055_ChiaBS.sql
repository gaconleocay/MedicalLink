...
FROM 
	(SELECT vp.patientid, 
		...
		((case ser.loaidoituong when 0 then ser.servicepricemoney_bhyt when 1 then ser.servicepricemoney_nhandan else ser.servicepricemoney end) * ser.soluong * 0.25) as mochinh_tien,
		pttt.bacsigayme as gayme_tenbs, 
		((case ser.loaidoituong when 0 then ser.servicepricemoney_bhyt when 1 then ser.servicepricemoney_nhandan else ser.servicepricemoney end) * ser.soluong * 0.05) as gayme_tien,			
