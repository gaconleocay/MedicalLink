...
FROM (select spt.vienphiid,  
		...
		where COALESCE(spt.vienphistatus_vp,0)=0 and spt.vienphistatus<>0 and spt.vienphidate_ravien>='" + this.KhoangThoiGianLayDuLieu + "' " + lstphongravien + _thutienstatus+_doituongbenhnhanid_spt+" group by spt.vienphiid) A ;
...
SELECT 
	(select count(vp.*) from vienphi vp where vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC + ") "+_doituongbenhnhanid_vp+") as raviendatt_slbn,
...
FROM 	
	...
	where spt.vienphistatus_vp=1 	
		and spt.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' " + lstphongravien + _thutienstatus+_doituongbenhnhanid_spt+" 
	...
	LEFT JOIN	
		...
		FROM 
				...
				FROM
					...
					from (select vp.vienphiid,vp.departmentgroupid from vienphi vp where vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC+") "+_doituongbenhnhanid_vp+") vp
		...
		SELECT 
				...
				(select  
						...
						ser.soluong,		
						vp.departmentgroupid
					from (select vp.vienphiid,vp.departmentgroupid from vienphi vp where vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC +") "+_doituongbenhnhanid_vp+") vp
...
FROM 
	...
	from (select 
			...
			from ihs_servicespttt spt 
			where spt.vienphistatus_vp=1 
				and spt.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' " + lstdepartmentid_tt + _thutienstatus+_doituongbenhnhanid_spt+" 
...
LEFT JOIN 	
	...
	from (select 
			...
			where spt.vienphistatus_vp=1 
				...
				and spt.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' "+_thutienstatus+_doituongbenhnhanid_spt+"
	...
	LEFT JOIN	
		...
		FROM 
				...
				FROM
					...
					from (select vp.vienphiid from vienphi vp where vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' "+_doituongbenhnhanid_vp+") vp
		...
		SELECT 
				...
				(select  
						...
						ser.soluong,		
						ser.departmentgroupid
					from (select vp.vienphiid from vienphi vp where vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "' "+_doituongbenhnhanid_vp+") vp
...
---====== BN dang dieu tri - ĐT BHYT + DV BHYT
--string sqlBaoCao_DangDT 
--ngay 14/6/2018
...
---====== BN ra vien nhung chua thanh toan - ĐT BHYT + DV BHYT
--string sqlBaoCao_RaVienChuaTT 
--ngay 14/6/2018
...
COALESCE(round(cast(sum(COALESCE(A.ravienchuatt_tienkb,0) + COALESCE(A.ravienchuatt_tienxn,0) + COALESCE(A.ravienchuatt_tiencdhatdcn,0) + COALESCE(A.ravienchuatt_tienpttt,0) + COALESCE(A.ravienchuatt_tiendvktc,0) + COALESCE(A.ravienchuatt_tiengiuongthuong,0) + COALESCE(A.ravienchuatt_tiengiuongyeucau,0) + COALESCE(A.ravienchuatt_tienkhac,0) + COALESCE(A.ravienchuatt_tienvattu,0) + COALESCE(A.ravienchuatt_tienmau,0) + COALESCE(A.ravienchuatt_tienthuoc,0) + COALESCE(A.ravienchuatt_tienvattu_ttrieng,0) + COALESCE(A.ravienchuatt_tienptttyeucau,0)) as numeric),0),0) as ravienchuatt_tongtien
FROM (select spt.vienphiid,  
		sum(spt.money_khambenh_bh) as ravienchuatt_tienkb,  
		sum(spt.money_xetnghiem_bh) as ravienchuatt_tienxn,  
		sum(spt.money_cdha_bh + spt.money_tdcn_bh) as ravienchuatt_tiencdhatdcn,  
		sum(spt.money_pttt_bh) as ravienchuatt_tienpttt,  
		sum(spt.money_ptttyeucau_bh) as ravienchuatt_tienptttyeucau, 
		sum(spt.money_dvktc_bh) as ravienchuatt_tiendvktc,  
		sum(spt.money_giuongthuong_bh) as ravienchuatt_tiengiuongthuong,  
		sum(spt.money_giuongyeucau_bh) as ravienchuatt_tiengiuongyeucau,  
		sum(spt.money_nuocsoi_bh + spt.money_xuatan_bh + spt.money_diennuoc_bh + spt.money_vanchuyen_bh + spt.money_khac_bh + spt.money_phuthu_bh) as ravienchuatt_tienkhac,  
		sum(spt.money_vattu_bh + spt.money_vtthaythe_bh) as ravienchuatt_tienvattu,  
		sum(spt.money_vattu_ttrieng_bh) as ravienchuatt_tienvattu_ttrieng,  
		sum(spt.money_mau_bh) as ravienchuatt_tienmau,  
		sum(spt.money_thuoc_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh) as ravienchuatt_tienthuoc 
		from ihs_servicespttt spt 
		where doituongbenhnhanid=1 and COALESCE(spt.vienphistatus_vp,0)=0 and spt.vienphistatus<>0 and spt.vienphidate_ravien>='" + this.KhoangThoiGianLayDuLieu + "' " + lstphongravien + _thutienstatus+" group by spt.vienphiid) A ;
		
		
	
-------RA VIEN DA THANH TOAN  - ĐT BHYT + DV BHYT
--string sqlBaoCao_RaVienDaTT =
--ngay 14/6/2018: them hao phi XN, CDHA, khoa
...
FROM 	
	(select 
			...
			sum(spt.money_vattu_bh + spt.money_vtthaythe_bh) as raviendatt_tienvattu, 
			sum(spt.money_vattu_ttrieng_bh) as raviendatt_tienvattu_ttrieng, 
			sum(spt.money_mau_bh) as raviendatt_tienmau, 
			sum(spt.money_thuoc_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh) as raviendatt_tienthuoc 
	from ihs_servicespttt spt 
	where spt.vienphistatus_vp=1 and spt.doituongbenhnhanid=1
	...
	LEFT JOIN	
		...
		FROM 
				...
				FROM
					(select  
						...
						se.idmayxn,
						vp.departmentgroupid
					from (select vp.vienphiid,vp.departmentgroupid from vienphi vp where vp.vienphistatus_vp=1 and vp.doituongbenhnhanid=1 and vp.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC + ")) vp
		...
		SELECT 
				...
				(select  
						...
						ser.soluong,		
						vp.departmentgroupid
					from (select vp.vienphiid,vp.departmentgroupid from vienphi vp where vp.vienphistatus_vp=1 and vp.doituongbenhnhanid=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC + ")) vp
...
--Chi phi CDHA 
	LEFT JOIN
		...
		FROM 
			...
			FROM (select vp.vienphiid,vp.departmentgroupid from vienphi vp where vp.vienphistatus_vp=1 and vp.doituongbenhnhanid=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "' and departmentid in (" + this.lstPhongChonLayBC + ")) vp		
				inner join (select vienphiid,departmentgroupid,servicepriceid,servicepricecode,loaidoituong,servicepricemoney_nhandan,servicepricemoney,loaipttt,soluong,chiphidauvao,chiphimaymoc,chiphipttt,mayytedbid from serviceprice where loaidoituong in (0,4,6) and bhyt_groupcode in ('04CDHA','05TDCN')) ser on ser.vienphiid=vp.vienphiid
				inner join (select servicepricecode,pttt_loaiid from servicepriceref where bhyt_groupcode in ('04CDHA','05TDCN')) serf on serf.servicepricecode=ser.servicepricecode) A
		GROUP BY A.departmentgroupid) CP_CDHA on CP_CDHA.departmentgroupid=A.khoaravien
--Chi phi khoa/phong
LEFT JOIN 
...
------ DOANH THU KHOA + GÂY MÊ   - ĐT BHYT + DV BHYT
-- string sqlDoanhThu
--ngay 15/6/2018: them hao phi XN, CDHA, khoa phong
...
FROM 
	...
	from (select 
				spt.departmentgroupid,spt.vienphiid,
				sum(spt.money_khambenh_bh) as doanhthu_tienkb, 
				sum(spt.money_xetnghiem_bh) as doanhthu_tienxn, 
				sum(spt.money_cdha_bh + spt.money_tdcn_bh) as doanhthu_tiencdhatdcn, 
				sum(spt.money_pttt_bh) as doanhthu_tienpttt,
				sum(spt.money_ptttyeucau_bh) as doanhthu_tienptttyeucau, 
				sum(spt.money_dvktc_bh) as doanhthu_tiendvktc, 
				sum(spt.money_giuongthuong_bh) as doanhthu_tiengiuongthuong, 
				sum(spt.money_giuongyeucau_bh) as doanhthu_tiengiuongyeucau, 
				sum(spt.money_nuocsoi_bh + spt.money_xuatan_bh + spt.money_diennuoc_bh + spt.money_vanchuyen_bh + spt.money_khac_bh + spt.money_phuthu_bh) as doanhthu_tienkhac, 
				sum(spt.money_vattu_bh + spt.money_vtthaythe_bh) as doanhthu_tienvattu, 
				sum(spt.money_vattu_ttrieng_bh) as doanhthu_tienvattu_ttrieng,
				sum(spt.money_mau_bh) as doanhthu_tienmau, 
				sum(spt.money_thuoc_bh + spt.money_dkpttt_thuoc_bh + spt.money_dkpttt_vattu_bh) as doanhthu_tienthuoc 
			from ihs_servicespttt spt 
			where spt.doituongbenhnhanid=1 and spt.vienphistatus_vp=1 
	...
	LEFT JOIN	
		...
		FROM 
				...
				FROM
					(select  
						...
						se.idmayxn,
						ser.departmentgroupid
					from (select vp.vienphiid from vienphi vp where vp.doituongbenhnhanid=1 and vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + this.thoiGianTu + "' and '" + this.thoiGianDen + "') vp
		...
		SELECT 
				...
				(select  
						...
						ser.soluong,		
						ser.departmentgroupid
					from (select vp.vienphiid from vienphi vp where vp.doituongbenhnhanid=1 and vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "') vp
...
--Chi phi CDHA 
	LEFT JOIN
		...
		FROM 
			...
			FROM (select vp.vienphiid from vienphi vp where vp.doituongbenhnhanid=1 and vp.vienphistatus_vp=1 and vp.duyet_ngayduyet_vp between '" + thoiGianTu + "' and '" + thoiGianDen + "') vp		
				inner join (select vienphiid,departmentgroupid,servicepriceid,servicepricecode,loaidoituong,servicepricemoney_nhandan,servicepricemoney,loaipttt,soluong,chiphidauvao,chiphimaymoc,chiphipttt,mayytedbid from serviceprice where loaidoituong in (0,4,6) and bhyt_groupcode in ('04CDHA','05TDCN') and departmentid in (" + this.lstPhongChonLayBC + ")) ser on ser.vienphiid=vp.vienphiid
				inner join (select servicepricecode,pttt_loaiid from servicepriceref where bhyt_groupcode in ('04CDHA','05TDCN')) serf on serf.servicepricecode=ser.servicepricecode) A
		GROUP BY A.departmentgroupid) CP_CDHA on CP_CDHA.departmentgroupid=A.departmentgroupid
--Chi phi khoa/phong
LEFT JOIN 
