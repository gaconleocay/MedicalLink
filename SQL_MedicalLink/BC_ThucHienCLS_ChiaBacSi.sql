--Bao cao Thuc hien Cận Lâm sàng ngay 29/3/2017
SELECT ROW_NUMBER () OVER (ORDER BY A.NGAY_THUCHIEN desc) as stt,
	...
	A.NGAY_THANHTOAN
FROM
(SELECT vp.patientid, vp.vienphiid, vp.hosobenhanid, vp.bhytid, ser.departmentgroupid as khoachidinh, 
	ser.departmentid as phongchidinh, ser.servicepricedate as NGAY_CHIDINH,
	...
	(case ser.loaipttt when 1 then 50.0
					   when 2 then 80.0
					   else 100.0 end) as TYLE,
	...
	(case serf.pttt_loaiid when 1 then 'Phẫu thuật đặc biệt' 
	...
	(case when serf.pttt_loaiid in (1,5) then 'x' 
						  else '' end) as LOAIPTTT_DB,
	(case when serf.pttt_loaiid in (2,6) then 'x' 
						  else '' end) as LOAIPTTT_L1,
	(case when serf.pttt_loaiid in (3,7) then 'x' 
						  else '' end) as LOAIPTTT_L2,
	(case when serf.pttt_loaiid in (4,8) then 'x' 
						  else '' end) as LOAIPTTT_L3,						  						 						  
	...
	((case serf.pttt_loaiid when 1 then 280000
							when 2 then 125000
						    when 3 then 56000
							when 4 then 50000
							when 5 then 84000
	...
	((case serf.pttt_loaiid when 1 then 200000
							when 2 then 90000
						    when 3 then 50000
							when 4 then 30000
							when 5 then 60000
	...
	((case serf.pttt_loaiid when 1 then 120000
							when 2 then 70000
						    when 3 then 30000
							when 4 then 15000
							when 5 then 36000
...
FROM 
	serviceprice ser left join thuchiencls cls on cls.servicepriceid=ser.servicepriceid
 ...
 WHERE serf.servicegrouptype=3 and serf.bhyt_groupcode in ('04CDHA','05TDCN') 
 and ser.bhyt_groupcode in ('04CDHA','05TDCN') 
 and " + serf_pttt_loaiid + " and " + ser_departmentid + " and " + tieuchi_date + " ) A
INNER JOIN maubenhpham mbp on mbp.maubenhphamid=A.maubenhphamid and mbp.maubenhphamgrouptype=1
