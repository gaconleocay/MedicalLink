...
namespace MedicalLink.TimerService
{
    public class TimerServiceProcess
    {
            ...
            try
            {
                if (chaymoidulieu == 0)
                {
                    ...
                    if (dataKiemTra != null && dataKiemTra.Count > 0)
                    {
                        ...
                        if ((datetime - thoigianUpdataTabl) > GlobalStore.ThoiGianCapNhatTbl_tools_bndangdt_tmp)
                        {
                            SQLChay_RaVienChuaTT_Tmp(dateTu, dateDen, dateKhoangDLTu, departmentgroupid);
                        }
                    }
                    else
                    {
        ...
        private static void SQLChay_RaVienChuaTT_Tmp(string dateTu, string dateDen, string dateKhoangDLTu, long departmentgroupid)
        {
            try
            {
                String datetimeNow = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
