...
{
    public partial class ucBC102_TrichThuongChuyenGiaYC : UserControl
    {
        #region Khai bao
        ...
        private string LstDanhMucDichVu { get; set; }
        private decimal TongTienChi = 0;
        #endregion

        public ucBC102_TrichThuongChuyenGiaYC()
        {
            InitializeComponent();
        }
        ...
        private void ucBC102_TrichThuongChuyenGiaYC_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lstOtherList != null && lstOtherList.Count > 0)
                {
                    ...
                    {
                        this.LstDanhMucDichVu += item.tools_otherlistvalue;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string tieuchi_ser = "";
                string tieuchi_vp = "";
                string lstdichvu_ser = " and servicepricecode in (" + this.LstDanhMucDichVu + ") ";
                string trangthai_vp = "";
                string sql_timkiem = "";
                string tieuchi_mbp = "";
                ...
                sql_timkiem = @"SELECT (row_number() OVER (PARTITION BY ncd.nhom_bcid ORDER BY ncd.username)) as stt, mbp.userid as userhisid, ncd.usercode, ncd.username, ncd.nhom_bcid, ncd.nhom_bcten, sum(ser.soluong) as soluong, sum(ser.soluong*ser.dongia) as thanhtien, 50 as tylehuong, sum(ser.soluong*ser.dongia*0.5) as tongtien, 0 as tienthue, 0 as thuclinh, '' as kynhan, 0 as isgroup FROM (select maubenhphamid,userid from maubenhpham where maubenhphamgrouptype=2 " + tieuchi_mbp + ") mbp inner join (select maubenhphamid,vienphiid,soluong,(case when doituongbenhnhanid=4 then servicepricemoney_nuocngoai else servicepricemoney_nhandan end) as dongia from serviceprice where bhyt_groupcode='01KB' " + lstdichvu_ser + tieuchi_ser + ") ser on ser.maubenhphamid=mbp.maubenhphamid inner join (select vienphiid from vienphi where 1=1 " + tieuchi_vp + trangthai_vp + ") vp on vp.vienphiid=ser.vienphiid left join (select * from nhompersonnel where nhom_bcid>0) ncd on ncd.userhisid=mbp.userid GROUP BY mbp.userid,ncd.usercode,ncd.username,ncd.nhom_bcid,ncd.nhom_bcten;";
        ...
        private void XuLyDuLieuVaHienThi(DataTable _dataBaocao)
        {
            try
            {
                ...
                foreach (var item_hd in _lst_HopDong)
                {
                    ...
                    item_hd.thuclinh = item_hd.tongtien ?? 0 - item_hd.tienthue ?? 0;
                ...
                //nhan vien benh vien
                List<TrichThuongChuyenGiaYCDTO> _lst_BenhVien = _lstBaoCao.Where(o => o.nhom_bcid == 2).ToList();
        ...
        private void tbnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                item_phong.value = this.LstDanhMucDichVu;
